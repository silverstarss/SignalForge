...
def _load_hf_model(config, model_config, is_value_model):
    ...
    with init_context(), warnings.catch_warnings():
        ...
        if "mistral7b-rm" in config.model.path:
            if MistralForSequenceClassification is None:
                raise ImportError(
                    "MistralForSequenceClassification is required only for mistral7b-rm model loading; "
                    "Qwen GRPO does not use this branch."
                )
