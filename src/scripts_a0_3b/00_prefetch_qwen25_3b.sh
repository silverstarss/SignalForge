...
LOCAL_DIR=${QWEN25_3B_LOCAL_DIR:-${MODEL_ROOT}/Qwen/Qwen2.5-3B-Instruct}
...
repo_id = os.environ.get("QWEN25_3B_REPO_ID", "Qwen/Qwen2.5-3B-Instruct")
local_dir = Path(os.environ.get("QWEN25_3B_LOCAL_DIR", "")).expanduser()
if not str(local_dir):
    model_root = Path(os.environ["MODEL_ROOT"])
    local_dir = model_root / "Qwen" / "Qwen2.5-3B-Instruct"
